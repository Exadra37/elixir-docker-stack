...
ARG DOCKER_ERLANG_VERSION=21.3.3-1
ARG DOCKER_ELIXIR_VERSION=1.8.1-2
...
ARG ERLANG_DOWNLOAD_URL=https://packages.erlang-solutions.com/erlang/esl-erlang/FLAVOUR_1_general/esl-erlang_"${DOCKER_ERLANG_VERSION}"~debian~stretch_amd64.deb
...
RUN apt update && \
  ...
  curl -o esl.deb "${ERLANG_DOWNLOAD_URL}" && \
  dpkg -i esl.deb && \
  rm -f esl.deb && \
